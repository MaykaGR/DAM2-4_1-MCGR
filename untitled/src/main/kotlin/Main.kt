...
class Modulo(
    ...
    /*fun matricular(al:Alumnos, Als: Array<Alumnos?>): Boolean {
        var matriALs= Als.filter {Null}
    ...
    /*fun matricular(al: Alumnos , Als: Array<Alumnos>) {
        var counter = 0
        return if (Als.size<=15){
         while (counter in 0 until Als.size) {
             if (Als[counter] == null)
               {  Als[counter] = al
             counter = Als.size}
         }
    ...
    /*fun darBaja(Als: Array<Alumnos>, ID){
        Als = Als.removeIf {ID.equals(it.ID)}
    }*/
}
...
fun main() {
    ...
    /*if (p1.matricular(c1))
        println("Alumno ${Als.Nombre} añadido correctamente")
    else
        println("Error añadiendo alumno ${Als.Nombre}")*/
    //var ordenada = Notafinal.sortedBy{ Matri -> Matri.Notas }
}
